...
def _read_model_evidence(settings_path: Path) -> dict[str, Any]:
    evidence: dict[str, Any] = {
        "source": str(settings_path),
        "model": "",
        "available": False,
        "control": "persistent-settings-only",
        "note": "AGY 1.0.1 exposes /model as a persistent setting; no per-call --model flag is available.",
    }
...
class AgyCliReviewer(BaseReviewer):
    def __init__(self, config: dict[str, Any]):
        ...
        self.model_evidence = _read_model_evidence(Path(config["settings_path"]))
    ...
    def _validate_model_expectation(self) -> None:
        ...
        actual = str(self.model_evidence.get("model") or "").strip()
        if expected and expected != actual:
            message = f"AGY 当前模型设置为 {actual or '未知'}，期望为 {expected}"
    ...
    def _build_prompt(self, *, code: str, day_chart: Path, prompt: str) -> str:
    ...
    def _run_agy(self, *, code: str, day_chart: Path, prompt_text: str) -> subprocess.CompletedProcess[str]:
        ...
        print(f"[Command] AGY CLI 实际命令: {self.agy_bin} --add-dir {day_chart.parent} --print-timeout {self.print_timeout} --print <prompt>")
        print(f"[INFO] AGY model evidence: {self.model_evidence.get('model') or 'unknown'}")
        if raw_dir is not None:
    ...
    def review_stock(self, code: str, day_chart: Path, prompt: str) -> dict:
        prompt_text = self._build_prompt(code=code, day_chart=day_chart, prompt=prompt)
    ...
    def run(self) -> None:
        ...
        for i, candidate in enumerate(candidates, 1):
            ...
            print(f"[{i}/{len(candidates)}] {code} — AGY 正在分析 ...", end=" ", flush=True)
            try:
