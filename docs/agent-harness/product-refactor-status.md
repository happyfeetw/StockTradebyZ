...
| R3 Core domain rewrite | Partial | #100 moved review suggestion logic into `src/stocktrade/domain/review/`; #112 split preselect into named ports with product-owned orchestration, CSV market loading, base market preparation, trading-date fallback, top-turnover pool construction, strategy dispatch, warmup bars, KDJ, ZX-line, weekly MA bull, max-volume filter, B2 price-action metrics, B2 volume confirmation, and recent-B1 lookup parity coverage | #112 still must replace remaining legacy selector formulas behind parity-tested ports |
