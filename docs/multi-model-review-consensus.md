...
- Z精选、Z观察、Z精选+观察、Z复盘样本：按 Z 质量层裁决导出精选池、观察池或复盘研究样本。Z 层使用 `z_select_min_quality_score` / `z_watch_min_quality_score` 二次质量分阈值，和模型复评 `suggest_min_score=4.0` 是不同口径。
- 分歧样本：至少一个模型推荐且至少一个模型不推荐。

导入前会按策略分组生成板块，板块名前缀区分来源，例如多数推荐使用 `CM`，共同推荐使用 `CA`；Z 相关方案统一在日期和策略之间插入 `Z`，例如 `0612ZB1`。自定义筛选可叠加 Z 裁决、Z 质量分、排除 Z 硬否决、排除 Z 观察限制、模型状态、推荐/观察/不推荐数量和共识分。导入链路复用正式结果中心的通达信导出能力，支持下载 `.bat`、浏览器写入和本地路径直写。
