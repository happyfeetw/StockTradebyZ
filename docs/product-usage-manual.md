...
当前下载实现复用 `pipeline.fetch_kline` 的抓取逻辑，但产品运行中心会把取消信号
传入下载循环。下载器最多只维持 `Workers` 个在途任务，取消后不再提交新的股票任务，
并会取消尚未启动的任务。已经进入单次 Tushare SDK 请求的线程不能被 Python 安全
强杀，会在该请求返回后的安全边界停止；重试等待和限流冷却等待会按秒检查取消信号。

运行详情顶部会显示结构化进度面板。每日数据下载按股票数展示下载进度；量化选股、
复评、归档、图表导出会展示当前阶段或已处理数量。进度同时保存在 run summary 中，
即使运行失败、取消或启动恢复，也能在详情页看到任务卡住的位置。
