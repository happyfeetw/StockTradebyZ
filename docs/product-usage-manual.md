...
已有初选结果和本次运行策略。运行计划区展示所选模式的旧 Workbench 等价命令，
用于验收步骤顺序和排障。点击“开始运行”会按所选模式串行触发对应产品 API；
点击“停止当前任务”会对当前 live run 发出取消请求，后端会在安全点收敛为
`cancelled`。

Workbench 工作流卡片下方是“运行观测”区。旧版中分散在下方独立运行列表、
每日数据表单、初选表单和运行详情里的能力已经收敛到这里。“查看运行记录”下拉框
不是流程配置项，而是观测选择器：用于选择最近某一次 product run，并查看该 run 的
状态、结构化进度、失败诊断和实时控制台日志。每个环节仍会创建独立 run，产物索引
仍保留在各自 run 下，通过产品 artifact API 访问。包含“量化初选”的模式会生成新的
初选结果，并自动用于后续图表、复评和归档；只有“只导出图表”和“只跑复评”这类复用
已有结果的模式才会显示“使用已有初选结果”。没有初选结果时，这些复用模式会阻塞，
要求先通过初选生成结果。
...
策略集合、初选结果、复评 provider、CSV 数量、候选数、推荐数等。它是验收和排障时
...
复评、归档、图表导出会展示当前阶段或已处理数量。复评 run 会展示“复评进度”，
优先读取 `summary.progress`；历史 run 若只有 `total_reviewed`、`total_candidates`
等复评摘要字段，前端也会推导出已复评/总候选进度。进度同时保存在 run summary 中，
...
会创建一份初选结果，并写入 SQLite；analytics writer 可同步写入 DuckDB。后端内部
仍使用 `candidate_batch_id` 作为血缘 ID，用于把候选、图表、复评和归档串起来；日常
产品界面用“初选结果/候选名单”表达这个概念。候选 identity 始终是 `(code, strategy)`。

### 6.3 图表导出

包含初选的流程会自动使用本次新生成的初选结果导出图表。若选择“只导出图表”，需要
在运行中心选择一份已有初选结果后点击“开始运行”。产品图表产物写入
`var/artifacts/{run_id}/` 并通过 artifact API 服务，不再依赖 legacy `data/kline/`
作为默认产品输出。

### 6.4 Gemini CLI 复评

包含初选的流程会自动使用本次新生成的初选结果复评。若选择“只跑复评”，需要在运行中心
选择一份已有初选结果后点击“开始运行”，发起 `provider=gemini-cli` 的复评。
要求：
...
包含初选和复评的流程会自动使用本次初选结果和本次复评结果归档。Archive 会把内部
candidate batch 和 review run 固化为日级归档快照，并把推荐状态、
rank、chart artifact link 和 review payload 写入 SQLite/DuckDB。

归档不是删除、移动或覆盖复评结果，而是把“这一次初选 + 这一次复评”写成一个可验收、
可回放的每日结果快照。未归档的复评结果仍保留在复评证据中，但不会出现在“每日结果”
列表里；归档完成后，才会成为每日结果和历史某日结果筛选的数据来源。

同一个选股日期可以有多次归档快照，例如策略或复评阈值调整后重新归档。页面默认按归档
创建时间展示较新的快照；如果需要精确定位某次归档，可使用 archive run id 过滤。这样
不会破坏历史日期筛选，反而能保留“当时验收的结果列表”和后续重跑之间的区别。

候选和复评不再作为主导航页面；它们是运行中心流程和每日结果详情中的证据，而不是
日常决策入口。每日结果页面为空时，会给出回到运行中心或进入设置页迁移旧数据的入口；
若页面为空，优先回运行中心检查流程计划和当前初选结果。
