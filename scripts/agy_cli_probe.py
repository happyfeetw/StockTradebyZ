...
def _agy_print_command(agy_bin: str, args: argparse.Namespace, prompt: str) -> list[str]:
    cmd = [agy_bin]
    if args.model:
...
def build_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="stocktradebyz-agy-probe-") as tmp:
        ...
        models_result = _run_command([agy_bin, "models"], cwd=probe_cwd, timeout_seconds=args.command_timeout)
        ...
        if not args.skip_json_probe and capabilities["has_print"]:
            json_probe = _run_command(
                _agy_print_command(agy_bin, args, JSON_PROBE_PROMPT),
                cwd=probe_cwd,
        ...
        if image_path:
            ...
            if args.model:
                image_cmd += ["--model", args.model]
            image_cmd += ["--print-timeout", args.print_timeout, "--print", image_prompt]
        ...
        report = {
            ...
            "models": _summarize_probe(models_result),
...
def main() -> int:
    ...
    parser.add_argument("--model", default=DEFAULT_MODEL, help="Model passed to agy --model")
