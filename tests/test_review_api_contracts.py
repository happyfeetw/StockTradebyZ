    ...
    async def test_review_run_api_records_historical_batch_reruns_and_duckdb_facts(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
                ...
                self.assertEqual(first_payload["run"]["summary"]["progress"]["label"], "复评进度")
                self.assertEqual(first_payload["run"]["summary"]["progress"]["mode"], "review")
                self.assertEqual(first_payload["run"]["summary"]["progress"]["current"], 2)
                self.assertEqual(first_payload["run"]["summary"]["progress"]["total"], 2)
                self.assertEqual(first_payload["run"]["summary"]["progress"]["percent"], 100.0)
    ...
    async def test_review_provider_run_uses_chart_artifact_lineage_and_duckdb_facts(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
                ...
                self.assertEqual(payload["run"]["summary"]["progress"]["label"], "复评进度")
                self.assertEqual(payload["run"]["summary"]["progress"]["mode"], "review")
                self.assertEqual(payload["run"]["summary"]["progress"]["current"], 2)
                self.assertEqual(payload["run"]["summary"]["progress"]["total"], 2)
                self.assertEqual(payload["run"]["summary"]["progress"]["percent"], 100.0)
